...
def generate_061(record: Record):
    # Si l'information est répétée, on génère plusieurs sous-zones au sein d'une même zone
    f061 = []
    if "$o au" in record.new043 and re.search("(art vidéo|art video)", record.fields.f600a.lower()) is not None:
        f061.append("$a av")
    if "$b er" in record.new043:
        f061.append("$a er")
    if "$b et" in record.new043:
        f061.append("$a et")
    if "émission télévisée" in record.fields.f145a.lower():
        f061.append("$a et")
    if "film" in record.fields.f145e.lower():
        f061.append("$a fi")
    if "$b sr" in record.new043:
        f061.append("$a sr")
    if "série télévisée" in record.fields.f145a.lower():
        f061.append("$a et")
    if "$o au" in record.new043 and re.search("série télévisée", record.fields.f600a_5mots.lower()) is not None:
        f061.append("$a st")
    if "$b st" in record.new043:
        f061.append("$a st")
    ...
    if "web-documentaire" in record.fields.f145e.lower():
        f061.append("$a wd")  
    if "$a fi" in f061 and (sru.record2fieldvalue(record.xml_init, "600").lower().startswith("$a documentaire") or "film documentaire" in sru.record2fieldvalue(record.xml_init, "600").lower()):
        f061.append("$b fd")
    if "$a fi" in f061 and (sru.record2fieldvalue(record.xml_init, "600").lower().startswith("$a fiction") or "film de fiction" in sru.record2fieldvalue(record.xml_init, "600").lower()):
        f061.append("$b ff")
    ...
    if "$a st" in f061 and (re.search("(série télévisée policière|série télévisée policière|série télévisée judiciaire)", record.fields.f600a.lower()) is not None):
        f061.append("$g stpj")
    if "$o au" in record.new043 and (re.search("court métrage", record.fields.f600a.lower()) is not None):
        f061.append("$k mc")
    if "$o au" in record.new043 and (re.search("long métrage", record.fields.f600a.lower()) is not None):     # ALERT : code "ml" pour "long métrage" ?
        f061.append("$k ml")
    if "$o au" in record.new043 and (re.search("moyen métrage", record.fields.f600a.lower()) is not None): 
        f061.append("$k mm")
...
def generate_062(record: Record):
    ...
    if "$a jv" in f062:
        for f600a in sru.record2fieldvalue(record.xml_init, "600$a").split("¤"):
            if re.search(reg, f600a.lower()) is None and re.search("série de jeux vidéo", f600a.lower()) is None:  # ALERT : ajout de la seconde condition, pour être cohérent avec la suite
                new630.append(f600a)
            elif re.search("série de jeux vidéo", f600a.lower()) is not None:
                new631.append(f600a)

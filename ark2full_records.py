    ...
    for field in listefieldsLiensAUT:
        ...
        for datafield in intermarc_record.xpath(path, namespaces=ns):
            ...
            if (nna not in listeNNA_AUT):
                ...
                url = "http://catalogue.bnf.fr/api/SRU?version=1.2&operation=searchRetrieve&query=aut.recordid%20all%20%22" + nna + "%22&recordSchema=" + format_BIB + "&maximumRecords=20&startRecord=1"
                records = etree.parse(url)
                for record in records.xpath("//srw:recordData/mxc:record",namespaces=ns):
                    record2file(record)
...
bib_records = open("noticesBIB.xml","w")
aut_records = open("noticesAUT.xml","w")
